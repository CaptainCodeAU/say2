import Foundation
import XCTest
@testable import SiriTTSClient

final class SiriTTSClientTests: XCTestCase {
            ...
            transport: { request in
                ...
                return (
                    ...
                    Self.response(
                        ...
                        headers: ["X-Siri-TTS-Engine": "siri"]
                    )
                )
            }
        )
    ...
    func testLongAudioCanDownloadDirectlyToAFile() async throws {
        let temporaryDirectory = FileManager.default.temporaryDirectory.appendingPathComponent(
            "siri-tts-client-test-\(UUID().uuidString)",
            isDirectory: true
        )
        ...
        let client = SiriTTSClient(
            configuration: .init(baseURL: URL(string: "http://localhost:8080")!),
            transport: { _ in throw SiriTTSClient.ClientError.invalidResponse },
    ...
    func testSurfacesHelperErrorMessage() async throws {
        ...
        } catch let error as SiriTTSClient.ClientError {
    ...
    func testRejectsInvalidConfigurationBeforeTransport() async {
        let client = SiriTTSClient(
            ...
            transport: { _ in
                ...
                throw SiriTTSClient.ClientError.invalidResponse
            }
        )
