...
| `--pitch MULTIPLIER`           | `0.5` through `2.0`; default: `1.0`                              | Set native pitch. **AV engine only** — Apple's Siri engine accepts this field but does not honor it; say2 prints a warning to stderr and still produces output if you set it while using `--engine siri` (the default).                                                            |
| `--volume NUMBER`              | `0` through `1`; default: `1.0`                                  | Set native volume. **AV engine only**, for the same reason as `--pitch`.                                                                                                                                                                                                            |
...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `synthesize --json`                   | `schemaVersion`, selected `engine`, optional `fallbackFrom`, `voice`, `audio`, `timings`, `timingsSupported`, `elapsedSeconds`, `timeToFirstAudioSeconds` (optional), and `ignoredOptions`. `voice` contains `name`, `language`, `assetKey`, `version`, `engine`, and `installed`. `audio` contains `format`, `output`, sample format, byte/sample counts, duration, and `nonSilent`. `ignoredOptions` lists options that were accepted but had no effect on this render (e.g. `"pitch"`/`"volume"` on the siri engine) — always present, empty when nothing was ignored, so a caller can assert on it without a nil check. |
